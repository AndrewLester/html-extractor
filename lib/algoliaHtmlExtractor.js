...
export default class AlgoliaHTMLExtractor {
    ...
    css(input, selector) {
        const tags = new Set(['h1','h2','h3','h4','h5','h6','p']);
        const dom = htmlparser2.parseDocument(input);
        return find((elem) => isTag(elem) && tags.has(elem.name), dom.children, true, Infinity);
    }
    ...
    extractAnchor(node) {
        const anchor =
            node.getAttribute('name') || node.getAttribute('id') || null;
