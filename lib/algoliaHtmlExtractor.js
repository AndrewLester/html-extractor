...
export default class AlgoliaHTMLExtractor {
    ...
    css(input, selector) {
        ...
        const nodes = find(
            (elem) => isTag(elem) && tags.has(elem.name),
            dom.children,
            true,
            Infinity
        );
    ...
    async run(input, options = {}) {
        ...
        await Promise.all(
            this.css(input, `${headingSelector},${cssSelector}`).map(
                async (node) => {
                    ...
                    if (tagsToExclude && tagsToExclude.length) {
                        find(
                            (elem) =>
                                isTag(elem) &&
                                new Set(tagsToExclude.split(',')).has(
                                    elem.name
                                ),
                            [node],
                            true,
                    ...
                    const item = {
                        ...
                        content,
                        headings: Object.values(currentHierarchy).filter(
                            (h) => h
                        ),
    ...
    extractAnchor(node) {
        ...
        const subelement = findOne(
            (elem) =>
                isTag(elem) &&
                (hasAttrib(elem, 'name') || hasAttrib(elem, 'id')),
            [node],
            true
        );
